const files_qaMathTree = [
  {
    folder: "大学数学",
    files: [
      ["大学数学_ガロア理論", 1],
    ],
  },
  {
    folder: "数学",
    files: [
      ["数学Ⅰ_数と式", 10],
      ["数学Ⅰ_数と式_集合と命題", 10],
      ["数学Ⅰ_数と式_不等式", 10],
      ["数学Ⅰ_図形と計量", 10],
      ["数学Ⅰ_二次関数", 10],
      ["数学Ⅰ_二次関数_グラフと最大最小", 10],
      ["数学Ⅰ_データの分析", 10],
      ["数学Ⅰ_データの分析_仮説検定の考え方", 10],
      ["数学A_場合の数と確率", 10],
      ["数学A_図形の性質", 10],
      ["数学A_図形の性質_作図と証明", 10],
      ["数学A_数学と人間の活動", 10],
      ["数学Ⅱ_いろいろな式", 10],
      ["数学Ⅱ_いろいろな式_複素数と高次方程式", 10],
      ["数学Ⅱ_図形と方程式", 10],
      ["数学Ⅱ_図形と方程式_軌跡と領域", 10],
      ["数学Ⅱ_指数関数_対数関数", 10],
      ["数学Ⅱ_三角関数", 10],
      ["数学Ⅱ_微分_積分の考え", 10],
      ["数学Ⅱ_微分_積分の考え_導関数の応用", 10],
      ["数学B_数列", 10],
      ["数学B_統計的な推測", 10],
      ["数学B_統計的な推測_区間推定と仮説検定", 10],
      ["数学B_数学と社会生活", 10],
      ["数学B_数学と社会生活_モデル化", 10],
      ["数学C_ベクトル", 10],
      ["数学C_平面上の曲線と複素数平面", 10],
      ["数学C_数学的な表現の工夫", 10],
      ["数学C_数学的な表現の工夫_行列と離散グラフ", 10],
      ["数学Ⅲ_極限", 10],
      ["数学Ⅲ_微分法", 10],
      ["数学Ⅲ_積分法", 10],
    ],
  },
  {
    folder: "化学",
    files: [
      ["化学基礎_化学と人間生活_物質の構成", 3],
      ["化学基礎_物質量と化学反応式", 3],
      ["化学基礎_酸塩基と酸化還元", 1],
      ["化学_物質の状態と平衡_気体", 1],
      ["化学_物質の状態と平衡_溶液", 3],
      ["化学_物質の変化と平衡_熱と電気", 3],
      ["化学_物質の変化と平衡_化学平衡と電離平衡", 3],
      ["化学_反応速度と化学平衡", 3],
      ["化学_電離平衡と緩衝液", 3],
      ["化学_有機化合物の性質_燃焼分析", 3],
      ["化学_有機化合物の性質_構造決定", 3],
      ["化学_有機化合物の性質_官能基", 3],
      ["化学_無機物質", 3],
      ["化学_無機物質_系統分析", 3],
      ["化学_高分子化合物", 3],
      ["化学_化学が果たす役割", 3],
    ],
  },
  {
    folder: "物理",
    files: [
      ["物理_力学", 3],
      ["物理_円運動_単振動", 3],
      ["物理_熱力学", 3],
      ["物理_熱力学_気体分子運動", 3],
      ["物理_波動", 3],
      ["物理_波動_音と光", 3],
      ["物理_電磁気", 3],
      ["物理_電磁誘導と交流", 3],
      ["物理_電磁波", 3],
      ["物理_原子物理", 3],
      ["物理_原子_原子核と放射線", 3],
    ],
  },
  {
    folder: "英語",
    files: [
      ["英語_英文法", 1],
      ["英語_重要例文和訳", 1],
      ["英語_英文解釈_構文判定", 3],
      ["英語_英語コミュニケーション_読解要約", 3],
      ["英語_論理表現_発信テンプレート", 3],
      ["英語_リスニング_要点把握", 3],
      ["英語_ライティング_段落構成", 3],
    ],
  },
  {
    folder: "国語",
    files: [
      ["国語_古文単語", 10],
      ["国語_現代文_評論語彙", 3],
      ["国語_古文文法_助動詞", 3],
      ["国語_漢文句法", 3],
      ["国語_論理国語_評論構造", 3],
      ["国語_古典探究_古文読解", 3],
      ["国語_古典探究_漢文読解", 3],
      ["国語_言語文化_文学読解", 3],
      ["国語_国語表現_小論文", 3],
    ],
  },
  {
    folder: "情報",
    files: [
      ["情報I_情報社会の問題解決", 1],
      ["情報I_コンピュータの仕組み", 1],
      ["情報I_プログラムの基礎", 1],
      ["情報I_情報通信ネットワーク", 1],
      ["情報I_情報セキュリティ", 1],
      ["情報I_情報通信ネットワークとセキュリティ", 1],
      ["情報I_情報デザインとメディア", 3],
      ["情報I_アルゴリズムとプログラム", 3],
      ["情報I_データ活用", 3],
      ["情報I_データ活用_収集整理整形", 3],
      ["情報I_データベース", 3],
      ["情報I_モデル化とシミュレーション", 3],
      ["情報I_AIと機械学習の基礎", 3],
    ],
  },
];

function flattenQaFiles(entries, prefix = "") { const files = []; entries.forEach((entry) => { if (Array.isArray(entry)) { const name = prefix ? `${prefix}/${entry[0]}` : entry[0]; files.push([name, entry[1]]); return; } const folder = entry && entry.folder; const children = entry && entry.files; if (!folder || !Array.isArray(children)) return; const nextPrefix = prefix ? `${prefix}/${folder}` : folder; files.push(...flattenQaFiles(children, nextPrefix)); }); return files; }

const files_qaMath = flattenQaFiles(files_qaMathTree);